...
function retry() {
  // Let ECPay retry when our database is temporarily unavailable. Signature
  // failures still receive ok() so a forged request cannot cause a retry storm.
...
async function handle(request: Request) {
  ...
  if (!configuredMerchantId || fields.MerchantID?.trim() !== configuredMerchantId) {
    console.error("[ecpay-callback] MerchantID verification failed");
    return ok();
  }
  ...
  try {
    ...
    console.error("[ecpay-callback] CheckMacValue calculation failed", error instanceof Error ? error.message : "unknown error");
    return ok();
  }
  ...
  if (!/^[A-Za-z0-9]{1,20}$/.test(merchantTradeNo) || !Number.isInteger(tradeAmount)) {
    console.error("[ecpay-callback] callback fields are invalid");
    return ok();
  }
  ...
  if (!baseUrl || !secret) {
    console.error("[ecpay-callback] database credentials are missing");
    return ok();
  }
  ...
  try {
    const response = await fetch(`${baseUrl}/rest/v1/rpc/record_ecpay_payment_callback`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        ...
        p_payment_date: fields.PaymentDate?.trim() || null,
        p_payment_type: fields.PaymentType?.trim() || null,
      }),
    });
    if (!response.ok) {
      console.error("[ecpay-callback] database callback RPC failed", response.status);
      return retry();
    }
  } catch (error) {
    console.error("[ecpay-callback] database callback request failed", error instanceof Error ? error.message : "unknown error");
    return retry();
  }
  return ok();
}
...
Deno.serve((request) => handle(request).catch((error) => {
  console.error("[ecpay-callback] unexpected error", error instanceof Error ? error.message : "unknown error");
  return retry();
}));
