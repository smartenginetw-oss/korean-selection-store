...
- 商城首頁與商品列表在尚無可售商品時會顯示明確的準備中狀態與重新整理入口，不會留下空白商品區塊；套用篩選後沒有結果則仍顯示清除條件提示。
...
- 廠商報價建立頁支援一張報價單最多 20 筆明細；每筆明細獨立驗證商品、規格、暫存商品、成本、MOQ 與數量，核准後可完整帶入採購單。
- 報價單狀態轉移已由資料庫防護：草稿只能送出為已收到／已拒絕，已收到才能核准，已核准只能透過採購單流程轉為已轉採購單；後台下拉只顯示合法下一步。
...
- 採購單狀態轉移已由資料庫防護：草稿可下單／取消、已下單可取消；部分到貨與已收貨只能由到貨驗收交易自動更新，已收貨／已取消不提供回退。
- `/admin/purchase-orders` 僅開放老闆、合夥人與全營運員工；採購單狀態包含草稿、已下單、部分到貨、已收貨與已取消，採購單與明細均啟用 RLS，匿名角色無 Data API 權限。
- 到貨驗收已接入：可在已下單／部分到貨採購單建立不可刪除的到貨批次，輸入每筆良品到貨與損耗／瑕疵數量；資料庫交易會鎖定採購明細、檢查剩餘數量，僅增加良品的 `inventory_levels.on_hand` 並寫入 `purchase_received` 異動，良品與損耗合計完成後自動更新部分到貨／已收貨狀態。
- 暫存商品沒有 SKU 規格，無法自動入庫；到貨頁會明確標示並阻止送出。採購單建立頁可一次新增最多 20 筆商品明細，到貨頁支援逐筆分批驗收並同步庫存狀態。
- 已核准報價可從「廠商報價」按「轉成採購單」帶入供應商、幣別、匯率與全部明細；建立採購單後會將來源報價標記為已轉採購單，並保留報價快照。
