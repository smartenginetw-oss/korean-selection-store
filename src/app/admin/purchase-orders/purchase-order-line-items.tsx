...
export function PurchaseOrderLineItems({ products, variants }: { products: Product[]; variants: Variant[] }) {
  ...
  return (
    <section className={styles.lineItems} aria-labelledby="purchase-line-items-title">
      <div className={styles.lineItemsHeading}>
        <div>
          <h3 id="purchase-line-items-title">採購明細</h3>
          ...
          return (
            <fieldset className={styles.lineItem} key={line.key}>
              ...
              <div className={styles.lineItemGrid}>
                ...
                <label>採購數量<input className="input" name="quantity" value={line.quantity} onChange={(event) => updateLine(line.key, { quantity: event.target.value })} type="number" min="1" step="1" required /></label>
              </div>
            </fieldset>
          );
        })}
