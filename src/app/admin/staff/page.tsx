...
const staffRoleOptions = [{ value: "partner", label: "合夥人" }, { value: "catalog_staff", label: "商品／庫存" }, { value: "order_staff", label: "訂單／客服" }, { value: "staff", label: "全營運員工" }, { value: "customer", label: "撤銷後台權限" }] as const;
...
export default async function AdminStaffPage({ searchParams }: { searchParams: Promise<{ status?: string; message?: string }> }) {
  ...
  return <>
    <div className={adminStyles.titleRow}><div><div className="eyebrow">團隊・老闆專用</div><h1 className="serif">團隊管理</h1></div><span className="badge badge-stock">老闆專用</span></div>
    ...
    <div className={styles.grid}>
      <section className={adminStyles.panel}><h2>授予／撤銷團隊權限</h2><p className={adminStyles.panelIntro}>請先在 Supabase Authentication 建立帳號，再輸入相同電子郵件。合夥人可共同管理品牌內容；撤銷後會恢復為一般會員，既有訂單不受影響。</p><form action={setStaffMemberAction} className={styles.form}><label>電子郵件<input className="input" name="email" type="email" required maxLength={254} placeholder="partner@example.com" /></label><div className={styles.actions}><RoundedSelect name="role" options={staffRoleOptions} defaultValue="partner" ariaLabel="角色" /><button className="button button-primary" type="submit">更新權限</button></div></form><div className={styles.roleCards}>{EMPLOYEE_ROLES.map((employeeRole) => <div className={styles.roleCard} key={employeeRole}><strong>{roleLabel(employeeRole)}</strong><span>{roleDescription(employeeRole)}</span></div>)}<div className={styles.roleCard}><strong>{roleLabel("partner")}</strong><span>{roleDescription("partner")}</span></div></div></section>
