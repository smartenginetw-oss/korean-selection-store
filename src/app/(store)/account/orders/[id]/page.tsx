...
export default async function MemberOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const orderResult = await supabase.from("orders").select("id, order_number, created_at, recipient_name, phone, postal_code, city, district, address_line, subtotal, discount_total, shipping_total, grand_total, currency, payment_status, fulfillment_status, order_status, stock_mode, customer_note").eq("id", id).maybeSingle();
  ...
  const [itemsResult, shipmentResult, timelineResult] = await Promise.all([
    supabase.from("order_items").select("product_name, variant_name, sku, unit_price, quantity, line_total, fulfillment_mode, selected_options").eq("order_id", id).order("created_at", { ascending: true }),
  ...
  return <div className={`container ${styles.page}`}>
    ...
    <div className={styles.layout}>
      <section className={`${styles.panel} ${styles.itemsPanel}`} aria-labelledby="items-heading"><h2 id="items-heading">商品明細</h2><div className={styles.items}>{items.map((item) => <div className={styles.item} key={`${item.sku}-${item.product_name}`}><div><h3>{item.product_name}</h3><p>{item.variant_name} · {item.sku} · 數量 {item.quantity}{optionText(item.selected_options) ? ` · ${optionText(item.selected_options)}` : ""}</p></div><strong>{formatTwd(item.line_total)}</strong></div>)}</div>{!items.length && <p className={styles.empty}>商品明細目前無法讀取。</p>}</section>
      ...
      <section className={styles.panel} aria-labelledby="delivery-heading"><h2 id="delivery-heading">配送資訊</h2><div className={styles.summary}><div className={styles.summaryRow}><span>收件人</span><strong>{order.recipient_name} · {order.phone}</strong></div><div className={styles.summaryRow}><span>宅配地址</span><strong>{order.postal_code} {order.city}{order.district}{order.address_line}</strong></div>{shipment && <div className={styles.summaryRow}><span>出貨資訊</span><strong>{shipment.carrier} · {shipment.tracking_number || "待填寫"}</strong></div>}</div></section>
